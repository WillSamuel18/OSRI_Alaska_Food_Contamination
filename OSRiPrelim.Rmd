...
write_xlsx(D7, "FullOSRIData_WDups_AllYears_Aug27.xlsx")

##Then take out the duplicates
...
D8<-D7%>%
  filter(!Study_name%in%c("Prince William Sound Regional Citizens Advisory Council","NOAA Prince William Sound RCAC 1993-2007","NOAA Resurrection Bay Intercomparison Project 2009", "Special Alaska Study","Pribilofs Special Study","AKMAP Special Study","Special Alaska Study","North Pacific Research Board Alaska Study"),
         Year_clean>1999,
...
write_xlsx(D8, "CleanPAH_OSRI_Data>1999_Aug27.xlsx")
```
...
```{r}
unique(D1$Parameter_standardized)

#I want to work with 'Common_name', 'Scientific_name','Species_standard', and 'Layman_group'
...
## Now Add in olde data set and see if the problem is there too
file_path <- "data/OSRI_data_D0.xlsx"
D0<- read_xlsx(file_path)
rows_with_na_specific_D0 <- D0[!complete.cases(D0[, columns_to_check]), ] # Number of rows is very similar. Take the D1 data set and handbomb the species names, fix missing parameter and lat long values
...
Plot some Data
```{r}
#values over Species standard, 
a<-ggplot(D0, aes(x = `Layman_group`, y = log(`Value_standardized`), color = `Layman_group`)) +
  ...
  labs(title = "All Hydrocarbon data across taxa", x = "", y = "PAH ng/g") +
  theme_minimal() +
  theme(
...
a
b<-ggplot(data=(D0%>%filter(Long<-140)), aes(x = Lat, y = filter(Long<-140))) +
  geom_point() +  # Add jittered points
  labs(title = "", x = "", y = "") +
  theme_minimal() +
  theme(
    legend.position = "none",  # Removes the legend
...
#Double check Nationwide dataset
ERM<-D1%>%
  filter(`Data_source`=="ERM")
summary(Nat)
unique(Nat$Parameter_standardized)
```